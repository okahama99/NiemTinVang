...
public interface BlueprintRepository extends JpaRepository<Blueprint, Long> {
    Page<Blueprint> findAllByStatusNotIn(
            Collection<Status> statusCollection, Pageable paging);

    Blueprint getByBlueprintNameAndStatusNotIn(
    ...
    Optional<Blueprint> findByBlueprintIdAndStatusNotIn(
            long blueprintId, Collection<Status> statusCollection);
    ...
    @Query("SELECT case when count(b) > 0 then true else false end " +
            "FROM Blueprint b " +
            "WHERE (b.projectId = ?1 or b.blueprintName = ?2) and b.blueprintId <> ?3 and b.status not in ?4 ")
    ...
    /* projectId */
    Optional<Blueprint> findByProjectIdAndStatusNotIn(
    ...
    @Query("SELECT case when count(b) > 0 then true else false end " +
            "FROM Blueprint b " +
            "WHERE (b.projectId = ?1 or b.blueprintName = ?2) and b.status not in ?3 ")
